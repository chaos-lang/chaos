...
char *type_strings[] = {"K_BOOL", "K_NUMBER", "K_STRING", "K_ANY",
                        "K_LIST", "K_DICT",   "K_VOID"};
...
void compile(char *module, enum Phase phase_arg, char *bin_file,
  ...
  if (bin_file != NULL) {
    sprintf(c_file_path, "%s%s%s.c", __KAOS_BUILD_DIRECTORY__,
            __KAOS_PATH_SEPARATOR__, bin_file);
    sprintf(h_file_path, "%s%s%s.h", __KAOS_BUILD_DIRECTORY__,
            __KAOS_PATH_SEPARATOR__, bin_file);
  } else {
    sprintf(c_file_path, "%s%smain.c", __KAOS_BUILD_DIRECTORY__,
            __KAOS_PATH_SEPARATOR__);
    sprintf(h_file_path, "%s%smain.h", __KAOS_BUILD_DIRECTORY__,
            __KAOS_PATH_SEPARATOR__);
  }
  ...
  if (keep) {
    fprintf(h_fp,
            "/*\n"
            " * Intermediate C header code generated by Chaos Programming "
            "Language\n"
            " * Language Reference: https://chaos-lang.org\n"
            " *\n");
    fprintf(c_fp,
            "/*\n"
            " * Intermediate C source code generated by Chaos Programming "
            "Language\n"
            " * Language Reference: https://chaos-lang.org\n"
            " *\n");
    if (bin_file != NULL) {
  ...
  fprintf(c_fp,
          ...
          indent, ' ', fixed_module_orig, indent, ' ', fixed_module_orig);
  ...
  if (bin_file != NULL) {
    ...
    sprintf(bin_file_path, "%s%s%s.exe", __KAOS_BUILD_DIRECTORY__,
            __KAOS_PATH_SEPARATOR__, bin_file);
#else
    sprintf(bin_file_path, "%s%s%s", __KAOS_BUILD_DIRECTORY__,
            __KAOS_PATH_SEPARATOR__, bin_file);
#endif
  } else {
#if defined(__clang__) && \
    (defined(_WIN32) || defined(_WIN64) || defined(__CYGWIN__))
    sprintf(bin_file_path, "%s%smain.exe", __KAOS_BUILD_DIRECTORY__,
            __KAOS_PATH_SEPARATOR__);
#else
    sprintf(bin_file_path, "%s%smain", __KAOS_BUILD_DIRECTORY__,
            __KAOS_PATH_SEPARATOR__);
#endif
  }
  ...
  sprintf(include_path, "%ls/LLVM/lib/clang/%d.%d.%d/include/chaos", szPath,
          __clang_major__, __clang_minor__, __clang_patchlevel__);
#elif defined(__GNUC__) || defined(__GNUG__)
  SHGetKnownFolderPath(&FOLDERID_ProgramData, 0, NULL, &szPath);
  sprintf(include_path,
          "%ls/Chocolatey/lib/mingw/tools/install/mingw64/lib/gcc/"
          "x86_64-w64-mingw32/%d.%d.%d/include/chaos",
  ...
  sprintf(include_path_windows_getopt, "\"%s/utilities/windows/getopt.c\"",
          include_path);
  char include_path_ast[PATH_MAX];
  sprintf(include_path_ast, "\"%s/ast/ast.c\"", include_path);
  char include_path_interpreter[PATH_MAX];
  sprintf(include_path_interpreter, "\"%s/interpreter/interpreter.c\"",
          include_path);
  ...
  sprintf(include_path_extension, "\"%s/interpreter/extension.c\"",
          include_path);
  ...
  sprintf(include_path_alternative, "\"%s/compiler/lib/alternative.c\"",
          include_path);
  ...
  sprintf(cmd,
#if !defined(__clang__)
          "/c %s %s %s %s %s %s %s %s %s -o %s %s %s %s %s %s %s %s %s %s %s "
          "%s %s %s %s %s %s %s %s",
#else
          "/c %s %s %s %s -o %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s "
          "%s %s %s",
#endif
          c_compiler_path, "-fcommon",
#if !defined(__clang__)
          "-Wl,--stack,4294967296", "-Werror", "-Wall", "-pedantic",
          "-fcompare-debug-second", "-D__USE_MINGW_ANSI_STDIO",
#else
          "-Wno-everything",
#endif
          "-DCHAOS_COMPILER", bin_file_path, c_file_path, include_path_helpers,
          include_path_language, include_path_cwalk,
          include_path_windows_getopt, include_path_ast,
          include_path_interpreter, include_path_errors, include_path_extension,
          include_path_function, include_path_module, include_path_symbol,
          include_path_alternative, include_path_parser,
          include_path_parser_tab, include_path_lex_yy, include_path_chaos,
          include_path_include);

  if (extra_flags != NULL)
    sprintf(cmd, "%s %s", cmd, extra_flags);

  if (CreateProcess("C:\\WINDOWS\\system32\\cmd.exe", cmd, NULL, NULL, TRUE, 0,
  ...
  c_compiler_args[arg_i++] =
      "/usr/local/include/chaos/interpreter/interpreter.c";
...
#if defined(_WIN32) || defined(_WIN64) || defined(__CYGWIN__)
                          c_compiler_args[arg_i + extra_flags_count] =
  ...
  if (!string_ends_with(bin_file_path, __KAOS_WINDOWS_EXE_EXT__)) {
    sprintf(bin_file_path_final, "%s%s", bin_file_path,
            __KAOS_WINDOWS_EXE_EXT__);
  } else {
...
transpile_functions_label:
  ...
  if (debug_enabled)
    printf(
        "(TranspileF)\tASTNode: {id: %llu, node_type: %s, module: %s, "
        "string_size: %zu}\n",
  ...
  if (ast_node->node_type >= AST_DEFINE_FUNCTION_BOOL &&
    ...
    function_name =
        snprintf_concat_string(function_name, "kaos_function_%s", module);
    function_name =
        snprintf_concat_string(function_name, "_%s", ast_node->strings[0]);
    if (!ast_node->dont_transpile &&
        !is_in_array(&transpiled_functions, function_name)) {
  ...
  case AST_DECISION_DEFINE:
    decision_name = NULL;
    decision_name =
        snprintf_concat_string(decision_name, "kaos_decision_%s", module);
    decision_name =
        snprintf_concat_string(decision_name, "_%s", last_function_name);
    if (!ast_node->dont_transpile &&
        !is_in_array(&transpiled_decisions, decision_name)) {
...
transpile_decisions_label:
  ...
  if (ast_node->node_type >= AST_DECISION_MAKE_BOOLEAN &&
      ast_node->node_type <= AST_DECISION_MAKE_DEFAULT_RETURN) {
  ...
  if (debug_enabled) {
    printf(
        "(TranspileD)\tASTNode: {id: %llu, node_type: %s, module: %s, "
        "string_size: %zu}\n",
        ast_node->id, getAstNodeTypeName(ast_node->node_type), ast_node->module,
        ast_node->strings_size);
  }
  ...
  case AST_DECISION_MAKE_BOOLEAN:
    if (ast_node->right->is_transpiled) {
      fprintf(c_fp,
              "%*cif (%s)\n"
              "%*c{\n",
              indent, ' ', ast_node->right->transpiled, indent, ' ');
    } else {
      fprintf(c_fp,
              "%*cif (%s)\n"
              "%*c{\n",
              indent, ' ', ast_node->right->value.b ? "true" : "false", indent,
              ' ');
    }
    fprintf(c_fp,
            "%*ccallFunctionCleanUpSymbols(function_call_stack.arr[function_"
            "call_stack.size - 1]);\n",
            indent + indent_length, ' ');
    switch (ast_node->strings_size) {
    case 1:
      fprintf(c_fp,
              "%*cFunctionCall* function_call_%llu = callFunction(\"%s\", "
              "function_call_stack.arr[function_call_stack.size - "
              "1]->function->module);\n",
              indent + indent_length, ' ', compiler_function_counter,
              ast_node->strings[0]);
      module_context = compiler_getFunctionModuleContext(ast_node->strings[0],
                                                         ast_node->module);
      transpile_function_call_decision(c_fp, ast_node->module, module_context,
                                       ast_node->strings[0],
                                       indent + indent_length);
      free(module_context);
      module_context = NULL;
    ...
    case 2:
      fprintf(c_fp,
              ...
              "\"%s\");\n",
              indent + indent_length, ' ', compiler_function_counter,
              ast_node->strings[1], ast_node->strings[0]);
      module_context = compiler_getFunctionModuleContext(ast_node->strings[1],
                                                         ast_node->strings[0]);
      transpile_function_call_decision(c_fp, ast_node->module, module_context,
                                       ast_node->strings[1],
                                       indent + indent_length);
    ...
    fprintf(c_fp,
            ...
            "%*c}\n",
            indent + indent_length, ' ', compiler_function_counter,
            indent + indent_length, ' ', indent + indent_length, ' ',
            compiler_function_counter, indent + indent_length, ' ', indent, ' ',
            indent + indent_length, ' ', indent, ' ');
    break;
  ...
  case AST_DECISION_MAKE_BOOLEAN_RETURN:
    ...
    fprintf(c_fp,
            ...
            "%*creturn;\n"
            "%*c}\n",
            indent + indent_length, ' ', ast_node->strings[0],
            indent + indent_length, ' ', indent + indent_length, ' ', indent,
            ' ');
    break;
  case AST_DECISION_MAKE_DEFAULT:
    fprintf(c_fp,
            "%*cif (function_call_stack.arr[function_call_stack.size - 1] != "
            "NULL)\n"
            "%*c{\n"
            "%*ccallFunctionCleanUpSymbols(function_call_stack.arr[function_"
            "call_stack.size - 1]);\n",
            indent, ' ', indent, ' ', indent + indent_length, ' ');
    switch (ast_node->strings_size) {
    case 1:
      fprintf(c_fp,
              "%*cFunctionCall* function_call_%llu = callFunction(\"%s\", "
    ...
    fprintf(c_fp,
            ...
            indent + indent_length, ' ', compiler_function_counter,
            indent + indent_length, ' ', indent + indent_length, ' ',
            compiler_function_counter, indent, ' ', indent + indent_length, ' ',
            indent, ' ');
    break;
  case AST_DECISION_MAKE_DEFAULT_BREAK:
    fprintf(c_fp,
            "%*cif (nested_loop_counter > 0 && "
            "function_call_stack.arr[function_call_stack.size - 1] != NULL)\n"
            "%*c{\n"
            "%*cdecisionBreakLoop();\n"
            "%*c}\n",
            indent, ' ', indent, ' ', indent + indent_length, ' ', indent, ' ');
    break;
  ...
  case AST_DECISION_MAKE_DEFAULT_RETURN:
    fprintf(c_fp,
            ...
            "%*c}\n",
            indent, ' ', indent, ' ', indent + indent_length, ' ',
            ast_node->strings[0], indent + indent_length, ' ', indent, ' ');
    break;
  default:
    break;
  }
...
compiler_register_functions_label:
  ...
  if ((ast_node->node_type >= AST_DEFINE_FUNCTION_BOOL &&
      ...
      (ast_node->node_type >= AST_FUNCTION_PARAMETER_BOOL &&
  ...
  if (debug_enabled) {
    printf(
        "(TranspileR)\tASTNode: {id: %llu, node_type: %s, module: %s, "
        "string_size: %zu}\n",
        ast_node->id, getAstNodeTypeName(ast_node->node_type), ast_node->module,
        ast_node->strings_size);
  }
  ...
  case AST_DEFINE_FUNCTION_BOOL:
    fprintf(c_fp,
            "%*cstartFunction(\"%s\", K_BOOL, K_ANY, \"%s\", \"%s\", \"%s\", "
            "false);\n",
            indent, ' ', ast_node->strings[0], compiler_current_context,
            compiler_current_module_context, compiler_current_module);
    break;
  case AST_DEFINE_FUNCTION_NUMBER:
    fprintf(c_fp,
            "%*cstartFunction(\"%s\", K_NUMBER, K_ANY, \"%s\", \"%s\", \"%s\", "
            "false);\n",
            indent, ' ', ast_node->strings[0], compiler_current_context,
            compiler_current_module_context, compiler_current_module);
    break;
  case AST_DEFINE_FUNCTION_STRING:
    fprintf(c_fp,
            "%*cstartFunction(\"%s\", K_STRING, K_ANY, \"%s\", \"%s\", \"%s\", "
            "false);\n",
            indent, ' ', ast_node->strings[0], compiler_current_context,
            compiler_current_module_context, compiler_current_module);
    break;
  case AST_DEFINE_FUNCTION_ANY:
    fprintf(c_fp,
            "%*cstartFunction(\"%s\", K_ANY, K_ANY, \"%s\", \"%s\", \"%s\", "
            "false);\n",
            indent, ' ', ast_node->strings[0], compiler_current_context,
            compiler_current_module_context, compiler_current_module);
    break;
  case AST_DEFINE_FUNCTION_LIST:
    fprintf(c_fp,
            "%*cstartFunction(\"%s\", K_LIST, K_ANY, \"%s\", \"%s\", \"%s\", "
            "false);\n",
            indent, ' ', ast_node->strings[0], compiler_current_context,
            compiler_current_module_context, compiler_current_module);
    break;
  case AST_DEFINE_FUNCTION_DICT:
    fprintf(c_fp,
            "%*cstartFunction(\"%s\", K_DICT, K_ANY, \"%s\", \"%s\", \"%s\", "
            "false);\n",
            indent, ' ', ast_node->strings[0], compiler_current_context,
            compiler_current_module_context, compiler_current_module);
    break;
  case AST_DEFINE_FUNCTION_BOOL_LIST:
    fprintf(c_fp,
            "%*cstartFunction(\"%s\", K_LIST, K_BOOL, \"%s\", \"%s\", \"%s\", "
            "false);\n",
            indent, ' ', ast_node->strings[0], compiler_current_context,
            compiler_current_module_context, compiler_current_module);
    break;
  case AST_DEFINE_FUNCTION_BOOL_DICT:
    fprintf(c_fp,
            "%*cstartFunction(\"%s\", K_DICT, K_BOOL, \"%s\", \"%s\", \"%s\", "
            "false);\n",
            indent, ' ', ast_node->strings[0], compiler_current_context,
            compiler_current_module_context, compiler_current_module);
    break;
  case AST_DEFINE_FUNCTION_NUMBER_LIST:
    fprintf(c_fp,
            "%*cstartFunction(\"%s\", K_LIST, K_NUMBER, \"%s\", \"%s\", "
            "\"%s\", false);\n",
            indent, ' ', ast_node->strings[0], compiler_current_context,
            compiler_current_module_context, compiler_current_module);
    break;
  case AST_DEFINE_FUNCTION_NUMBER_DICT:
    fprintf(c_fp,
            "%*cstartFunction(\"%s\", K_DICT, K_NUMBER, \"%s\", \"%s\", "
            "\"%s\", false);\n",
            indent, ' ', ast_node->strings[0], compiler_current_context,
            compiler_current_module_context, compiler_current_module);
    break;
  case AST_DEFINE_FUNCTION_STRING_LIST:
    fprintf(c_fp,
            "%*cstartFunction(\"%s\", K_LIST, K_STRING, \"%s\", \"%s\", "
            "\"%s\", false);\n",
            indent, ' ', ast_node->strings[0], compiler_current_context,
            compiler_current_module_context, compiler_current_module);
    break;
  case AST_DEFINE_FUNCTION_STRING_DICT:
    fprintf(c_fp,
            "%*cstartFunction(\"%s\", K_DICT, K_STRING, \"%s\", \"%s\", "
            "\"%s\", false);\n",
            indent, ' ', ast_node->strings[0], compiler_current_context,
            compiler_current_module_context, compiler_current_module);
    break;
  case AST_DEFINE_FUNCTION_VOID:
    fprintf(c_fp,
            "%*cstartFunction(\"%s\", K_VOID, K_ANY, \"%s\", \"%s\", \"%s\", "
            "false);\n",
            indent, ' ', ast_node->strings[0], compiler_current_context,
            compiler_current_module_context, compiler_current_module);
    break;
  ...
  case AST_OPTIONAL_FUNCTION_PARAMETER_LIST:
    fprintf(c_fp,
            ...
            indent, ' ', indent, ' ', ast_node->strings[0]);
    break;
  ...
  case AST_MODULE_IMPORT:
    ...
    break;
  case AST_MODULE_IMPORT_AS:
    compiler_handleModuleImportRegister(ast_node->strings[0], false, c_fp,
                                        indent);
    break;
  case AST_MODULE_IMPORT_PARTIAL:
...
transpile_node_label:
  ...
  kaos_lineno = ast_node->lineno;
  if ((ast_node->node_type >= AST_START_TIMES_DO &&
       ast_node->node_type <= AST_START_FOREACH_DICT) ||
      (ast_node->node_type >= AST_VAR_CREATE_BOOL &&
       ast_node->node_type <= AST_PRETTY_ECHO_VAR_EL) ||
      (ast_node->node_type >= AST_DELETE_VAR &&
       ast_node->node_type <= AST_EXIT_VAR) ||
      (ast_node->node_type == AST_PRINT_FUNCTION_TABLE) ||
      (ast_node->node_type >= AST_FUNCTION_CALL_PARAMETER_BOOL &&
       ast_node->node_type <= AST_FUNCTION_CALL_PARAMETER_DICT) ||
      (ast_node->node_type >= AST_PRINT_FUNCTION_RETURN &&
       ast_node->node_type <= AST_FUNCTION_RETURN) ||
  ...
  if (debug_enabled) {
    printf(
        "(Transpile)\tASTNode: {id: %llu, node_type: %s, module: %s, "
        "string_size: %zu}\n",
        ast_node->id, getAstNodeTypeName(ast_node->node_type), ast_node->module,
        ast_node->strings_size);
  }
  ...
  case AST_START_TIMES_DO:
    ...
    if (ast_node->right->is_transpiled) {
      ...
      fprintf(c_fp,
              ...
              "%*c{\n",
              indent, ' ', ast_node->right->value.i, indent, ' ');
    }
    indent += indent_length;
  ...
  case AST_START_TIMES_DO_VAR:
    ...
    fprintf(c_fp,
            ...
            "%*c{\n",
            indent, ' ', ast_node->strings[0], indent, ' ');
    indent += indent_length;
    fprintf(c_fp, "%*cif (setjmp(LoopBreak)) break;\n", indent, ' ');
    fprintf(c_fp, "%*cif (setjmp(LoopContinue)) continue;\n", indent, ' ');
    ast_node = transpile_node(ast_node->next, module, c_fp, indent);
  ...
  case AST_START_FOREACH:
    ...
    fprintf(c_fp,
            ...
            "%*cif (loop_%llu_list->type != K_LIST) throw_error(E_NOT_A_LIST, "
            "\"%s\");\n"
            "%*cfor (unsigned long i = 0; i < loop_%llu_list->children_count; "
            "i++)\n"
            "%*c{\n"
            "%*cSymbol* loop_%llu_child = loop_%llu_list->children[i];\n"
            "%*cSymbol* loop_%llu_clone_symbol = createCloneFromSymbol(\"%s\", "
            "loop_%llu_child->type, loop_%llu_child, "
            "loop_%llu_child->secondary_type);\n",
            indent, ' ', compiler_loop_counter, ast_node->strings[0], indent,
            ' ', compiler_loop_counter, ast_node->strings[0], indent, ' ',
            compiler_loop_counter, indent, ' ', indent + indent_length, ' ',
            compiler_loop_counter, compiler_loop_counter,
            indent + indent_length, ' ', compiler_loop_counter,
            ast_node->strings[1], compiler_loop_counter, compiler_loop_counter,
            compiler_loop_counter);
    indent += indent_length;
    fprintf(c_fp,
            ...
            "%*c}\n",
            indent, ' ', indent, ' ', indent + indent_length, ' ',
            compiler_loop_counter, indent + indent_length, ' ', indent, ' ');
    fprintf(c_fp,
  ...
  case AST_START_FOREACH_DICT:
    ...
    fprintf(c_fp,
            ...
            "%*cif (loop_%llu_dict->type != K_DICT) throw_error(E_NOT_A_DICT, "
            "\"%s\");\n"
            "%*cfor (unsigned long i = 0; i < loop_%llu_dict->children_count; "
            "i++)\n"
            "%*c{\n"
            ...
            compiler_loop_counter, indent + indent_length, ' ',
            compiler_loop_counter, ast_node->strings[2], compiler_loop_counter,
            compiler_loop_counter, compiler_loop_counter);
    indent += indent_length;
    fprintf(c_fp,
            "%*cif (setjmp(LoopBreak))\n"
            ...
            indent, ' ', indent, ' ', indent + indent_length, ' ',
            compiler_loop_counter, indent + indent_length, ' ',
            ast_node->strings[1], indent + indent_length, ' ', indent, ' ');
    fprintf(c_fp,
            "%*cif (setjmp(LoopContinue))\n"
            "%*c{\n"
            "%*cremoveSymbol(loop_%llu_clone_symbol);\n"
    ...
    if (ast_node->strings[0] == NULL) {
      ...
      if (ast_node->right->is_transpiled) {
        fprintf(c_fp, "%*caddSymbolBool(\"%s\", %s);\n", indent, ' ',
                ast_node->strings[0], ast_node->right->transpiled);
      } else {
        fprintf(c_fp, "%*caddSymbolBool(\"%s\", %s);\n", indent, ' ',
                ast_node->strings[0],
                ast_node->right->value.b ? "true" : "false");
  ...
  case AST_VAR_CREATE_BOOL_VAR:
    fprintf(c_fp,
            "%*ccreateCloneFromSymbolByName(\"%s\", K_BOOL, \"%s\", K_ANY);\n",
            indent, ' ', ast_node->strings[0], ast_node->strings[1]);
    break;
  case AST_VAR_CREATE_BOOL_VAR_EL:
    fprintf(
        c_fp,
        "%*ccreateCloneFromComplexElement(\"%s\", K_BOOL, \"%s\", K_ANY);\n",
        indent, ' ', ast_node->strings[0], ast_node->strings[1]);
    break;
      ...
      if (ast_node->right->value_type == V_INT) {
        if (ast_node->strings[0] == NULL) {
          ...
          fprintf(c_fp, "%*caddSymbolInt(\"%s\", %lld);\n", indent, ' ',
                  ast_node->strings[0], ast_node->right->value.i);
        }
      } else {
        if (ast_node->strings[0] == NULL) {
          ...
          fprintf(c_fp, "%*caddSymbolFloat(\"%s\", %Lf);\n", indent, ' ',
                  ast_node->strings[0], ast_node->right->value.f);
        }
      }
    }
    break;
  case AST_VAR_CREATE_NUMBER_VAR:
    fprintf(
        c_fp,
        "%*ccreateCloneFromSymbolByName(\"%s\", K_NUMBER, \"%s\", K_ANY);\n",
        indent, ' ', ast_node->strings[0], ast_node->strings[1]);
    break;
  case AST_VAR_CREATE_NUMBER_VAR_EL:
    fprintf(
        c_fp,
        "%*ccreateCloneFromComplexElement(\"%s\", K_NUMBER, \"%s\", K_ANY);\n",
        indent, ' ', ast_node->strings[0], ast_node->strings[1]);
    break;
  ...
  case AST_VAR_CREATE_STRING_VAR:
    fprintf(
        c_fp,
        "%*ccreateCloneFromSymbolByName(\"%s\", K_STRING, \"%s\", K_ANY);\n",
        indent, ' ', ast_node->strings[0], ast_node->strings[1]);
    break;
  case AST_VAR_CREATE_STRING_VAR_EL:
    fprintf(
        c_fp,
        "%*ccreateCloneFromComplexElement(\"%s\", K_STRING, \"%s\", K_ANY);\n",
        indent, ' ', ast_node->strings[0], ast_node->strings[1]);
    break;
  ...
  case AST_VAR_CREATE_ANY_VAR:
    fprintf(c_fp,
            "%*ccreateCloneFromSymbolByName(\"%s\", K_ANY, \"%s\", K_ANY);\n",
            indent, ' ', ast_node->strings[0], ast_node->strings[1]);
    break;
  case AST_VAR_CREATE_ANY_VAR_EL:
    fprintf(c_fp,
            "%*ccreateCloneFromComplexElement(\"%s\", K_ANY, \"%s\", K_ANY);\n",
            indent, ' ', ast_node->strings[0], ast_node->strings[1]);
    break;
  ...
  case AST_VAR_CREATE_LIST_VAR:
    fprintf(c_fp,
            "%*ccreateCloneFromSymbolByName(\"%s\", K_LIST, \"%s\", K_ANY);\n",
            indent, ' ', ast_node->strings[0], ast_node->strings[1]);
    break;
  ...
  case AST_VAR_CREATE_DICT_VAR:
    fprintf(c_fp,
            "%*ccreateCloneFromSymbolByName(\"%s\", K_DICT, \"%s\", K_ANY);\n",
            indent, ' ', ast_node->strings[0], ast_node->strings[1]);
    break;
  ...
  case AST_VAR_CREATE_BOOL_LIST_VAR:
    fprintf(c_fp,
            "%*ccreateCloneFromSymbolByName(\"%s\", K_LIST, \"%s\", K_BOOL);\n",
            indent, ' ', ast_node->strings[0], ast_node->strings[1]);
    break;
  ...
  case AST_VAR_CREATE_BOOL_DICT_VAR:
    fprintf(c_fp,
            "%*ccreateCloneFromSymbolByName(\"%s\", K_DICT, \"%s\", K_BOOL);\n",
            indent, ' ', ast_node->strings[0], ast_node->strings[1]);
    break;
  ...
  case AST_VAR_CREATE_NUMBER_LIST_VAR:
    fprintf(
        c_fp,
        "%*ccreateCloneFromSymbolByName(\"%s\", K_LIST, \"%s\", K_NUMBER);\n",
        indent, ' ', ast_node->strings[0], ast_node->strings[1]);
    break;
  ...
  case AST_VAR_CREATE_NUMBER_DICT_VAR:
    fprintf(
        c_fp,
        "%*ccreateCloneFromSymbolByName(\"%s\", K_DICT, \"%s\", K_NUMBER);\n",
        indent, ' ', ast_node->strings[0], ast_node->strings[1]);
    break;
  ...
  case AST_VAR_CREATE_STRING_LIST_VAR:
    fprintf(
        c_fp,
        "%*ccreateCloneFromSymbolByName(\"%s\", K_LIST, \"%s\", K_STRING);\n",
        indent, ' ', ast_node->strings[0], ast_node->strings[1]);
    break;
  ...
  case AST_VAR_CREATE_STRING_DICT_VAR:
    fprintf(
        c_fp,
        "%*ccreateCloneFromSymbolByName(\"%s\", K_DICT, \"%s\", K_STRING);\n",
        indent, ' ', ast_node->strings[0], ast_node->strings[1]);
    break;
  ...
  case AST_VAR_UPDATE_FUNC_RETURN:
    ...
    case 2:
      fprintf(
          ...
          indent, ' ', compiler_function_counter, ast_node->strings[1]);
    ...
    case 3:
      fprintf(c_fp,
              "%*cFunctionCall* function_call_%llu = callFunction(\"%s\", "
              "\"%s\");\n",
              indent, ' ', compiler_function_counter, ast_node->strings[2],
              ast_node->strings[1]);
    ...
    fprintf(
        ...
        "%*cupdateDecisionSymbolChainScope();\n"
        "%*cfree(function_call_%llu);\n",
        indent, ' ', ast_node->strings[0], compiler_function_counter, indent,
        ' ', indent, ' ', compiler_function_counter);
    break;
  case AST_RETURN_VAR:
  ...
  case AST_PRINT_COMPLEX_EL:
    fprintf(c_fp,
            ...
            "%*cgetComplexElementBySymbolId(variable_complex_element, "
            "variable_complex_element_symbol_id),\n"
            "%*cfalse,\n"
            "%*cfalse\n"
            "%*c);\n",
            indent, ' ', indent + indent_length, ' ', indent + indent_length,
            ' ', indent + indent_length, ' ', indent, ' ');
    break;
  ...
  case AST_COMPLEX_EL_UPDATE_VAR_EL:
    fprintf(c_fp,
            ...
            "%*c);\n",
            indent, ' ', indent + indent_length, ' ', ast_node->strings[0],
            indent, ' ');
    break;
  case AST_COMPLEX_EL_UPDATE_FUNC_RETURN:
    ...
    fprintf(
        ...
        "%*cupdateDecisionSymbolChainScope();\n"
        "%*cfree(function_call_%llu);\n",
        indent, ' ', compiler_function_counter, indent, ' ', indent, ' ',
        compiler_function_counter);
    break;
  case AST_PRINT_VAR:
    fprintf(
        c_fp,
        "%*cprintSymbolValueEndWithNewLine(getSymbol(\"%s\"), false, true);\n",
        indent, ' ', ast_node->strings[0]);
    break;
  ...
  case AST_ECHO_VAR:
    fprintf(
        c_fp,
        "%*cprintSymbolValueEndWith(getSymbol(\"%s\"), \"\", false, true);\n",
        indent, ' ', ast_node->strings[0]);
    break;
  ...
  case AST_PRETTY_PRINT_VAR:
    fprintf(
        c_fp,
        "%*cprintSymbolValueEndWithNewLine(getSymbol(\"%s\"), true, true);\n",
        indent, ' ', ast_node->strings[0]);
    break;
  ...
  case AST_PRETTY_ECHO_VAR:
    fprintf(
        c_fp,
        "%*cprintSymbolValueEndWith(getSymbol(\"%s\"), \"\", true, true);\n",
        indent, ' ', ast_node->strings[0]);
    break;
  ...
  case AST_PARENTHESIS:
    if (ast_node->right->is_transpiled) {
      setASTNodeTranspiled(ast_node,
                           snprintf_concat_string(ast_node->transpiled, "(%s)",
                                                  ast_node->right->transpiled));
    } else {
  ...
  case AST_VAR_EXPRESSION_VALUE:
    setASTNodeTranspiled(ast_node,
                         snprintf_concat_string(ast_node->transpiled,
                                                "getSymbolValueInt(\"%s\")",
                                                ast_node->strings[0]));
    ast_node->value_type = V_INT;
    break;
  case AST_VAR_EXPRESSION_INCREMENT:
    if (ast_node->right->is_transpiled) {
      setASTNodeTranspiled(
          ast_node, snprintf_concat_string(ast_node->transpiled, "%s + 1",
                                           ast_node->right->transpiled));
    } else {
  ...
  case AST_VAR_EXPRESSION_INCREMENT_ASSIGN:
    setASTNodeTranspiled(ast_node,
                         snprintf_concat_string(ast_node->transpiled,
                                                "incrementThenAssign(\"%s\"",
                                                ast_node->strings[0]));
    setASTNodeTranspiled(ast_node,
                         snprintf_concat_int(ast_node->transpiled, ", %lld)",
                                             ast_node->right->value.i));
    ast_node->value_type = V_INT;
    break;
  ...
  case AST_MIXED_EXPRESSION_DIVIDE:
    if (!transpile_common_mixed_operator(ast_node, "/")) {
      ...
      if (r_value == 0.0) {
        setASTNodeTranspiled(
            ast_node,
            snprintf_concat_string(ast_node->transpiled, "%s", "INFINITY"));
        break;
      }
      ast_node->value.f = l_value / r_value;
    }
  ...
  case AST_BOOLEAN_EXPRESSION_REL_EQUAL_UNKNOWN:
    setASTNodeTranspiled(
        ast_node, snprintf_concat_string(ast_node->transpiled,
  ...
  case AST_LEFT_RIGHT_BRACKET_EXPRESSION:
    ...
    if (ast_node->right->is_transpiled) {
      if (ast_node->right->value_type == V_INT) {
        fprintf(c_fp, "%*cSymbol* symbol_%llu = addSymbolInt(NULL, %s);\n",
                indent, ' ', compiler_symbol_counter,
                ast_node->right->transpiled);
      } else {
        fprintf(c_fp, "%*cSymbol* symbol_%llu = addSymbolFloat(NULL, %s);\n",
                indent, ' ', compiler_symbol_counter,
                ast_node->right->transpiled);
      }
    } else {
      if (ast_node->right->value_type == V_INT) {
        fprintf(c_fp, "%*cSymbol* symbol_%llu = addSymbolInt(NULL, %lld);\n",
                indent, ' ', compiler_symbol_counter, ast_node->right->value.i);
      } else {
        fprintf(c_fp, "%*cSymbol* symbol_%llu = addSymbolFloat(NULL, %Lf);\n",
                indent, ' ', compiler_symbol_counter, ast_node->right->value.f);
      }
    }
    fprintf(c_fp, "%*csymbol_%llu->sign = 1;\n", indent, ' ',
            compiler_symbol_counter);
    fprintf(c_fp, "%*cpushLeftRightBracketStack(symbol_%llu->id);\n", indent,
            ' ', compiler_symbol_counter);
  ...
  case AST_LEFT_RIGHT_BRACKET_MINUS_EXPRESSION:
    ...
    fprintf(c_fp,
            ...
            indent, ' ', compiler_symbol_counter, indent, ' ',
            compiler_symbol_counter, indent, ' ');
    break;
  case AST_LEFT_RIGHT_BRACKET_STRING:
    fprintf(c_fp, "%*cdisable_complex_mode = true;\n", indent, ' ');
    compiler_symbol_counter++;
    fprintf(c_fp, "%*cSymbol* symbol_%llu = addSymbolString(NULL, \"%s\");\n",
            indent, ' ', compiler_symbol_counter, ast_node->value.s);
    fprintf(c_fp,
            "%*csymbol_%llu->sign = 1;\n"
            "%*cpushLeftRightBracketStack(symbol_%llu->id);\n"
            "%*cdisable_complex_mode = false;\n",
  ...
  case AST_LEFT_RIGHT_BRACKET_VAR:
    ...
    compiler_symbol_counter++;
    fprintf(c_fp,
            "%*cSymbol* symbol_%llu = createCloneFromSymbolByName(NULL, K_ANY, "
            "\"%s\", K_ANY);\n",
            indent, ' ', compiler_symbol_counter, ast_node->strings[0]);
    fprintf(c_fp,
            "%*csymbol_%llu->sign = 1;\n"
            "%*cpushLeftRightBracketStack(symbol_%llu->id);\n"
            "%*cdisable_complex_mode = false;\n",
  ...
  case AST_EXIT_EXPRESSION:
    if (ast_node->right->is_transpiled) {
      fprintf(c_fp,
              ...
              "%*cfreeEverything();\n"
              "%*cexit(exit_code);\n",
              indent, ' ', ast_node->right->transpiled, indent, ' ', indent,
              ' ');
    } else {
      fprintf(c_fp,
              ...
              "%*cfreeEverything();\n"
              "%*cexit(exit_code);\n",
              indent, ' ', ast_node->right->value.i, indent, ' ', indent, ' ');
    }
    break;
  case AST_EXIT_VAR:
    fprintf(c_fp,
            ...
            "%*cfreeEverything();\n"
            "%*cexit(exit_code);\n",
            indent, ' ', ast_node->strings[0], indent, ' ', indent, ' ');
    break;
  ...
  case AST_JSON_PARSER:
    ...
    fprintf(c_fp,
            ...
            indent, ' ', indent, ' ', compiler_symbol_counter, indent, ' ');
    break;
  default:
    break;
  }
...
bool transpile_common_operator(ASTNode *ast_node, char *operator,
  ...
  if ((ast_node->left != NULL && ast_node->left->is_transpiled) ||
      (ast_node->right != NULL && ast_node->right->is_transpiled)) {
    if (ast_node->left != NULL && ast_node->left->is_transpiled) {
      if (in(operator, relational_operators, relational_operators_size)) {
        setASTNodeTranspiled(
            ast_node,
            snprintf_concat_string(ast_node->transpiled, "(long double) %s",
                                   ast_node->left->transpiled));
      } else {
        setASTNodeTranspiled(
            ast_node, snprintf_concat_string(ast_node->transpiled, "%s",
                                             ast_node->left->transpiled));
      }
    } else {
      ...
      case V_BOOL:
        if (in(operator, relational_operators, relational_operators_size)) {
          setASTNodeTranspiled(
              ast_node, snprintf_concat_int(ast_node->transpiled, "%lld",
                                            ast_node->left->value.b ? 1 : 0));
        } else {
          setASTNodeTranspiled(ast_node,
                               snprintf_concat_string(
                                   ast_node->transpiled, "%s",
                                   ast_node->left->value.b ? "true" : "false"));
        }
        break;
      case V_INT:
        if (in(operator, logical_operators, logical_operators_size)) {
          setASTNodeTranspiled(
              ast_node, snprintf_concat_string(
                            ast_node->transpiled, "%s",
                            ast_node->left->value.i > 0 ? "true" : "false"));
        } else {
          setASTNodeTranspiled(ast_node,
                               snprintf_concat_int(ast_node->transpiled, "%lld",
                                                   ast_node->left->value.i));
        }
        break;
      case V_FLOAT:
        if (in(operator, logical_operators, logical_operators_size)) {
          setASTNodeTranspiled(
              ast_node, snprintf_concat_string(
                            ast_node->transpiled, "%s",
                            ast_node->left->value.f > 0.0 ? "true" : "false"));
        } else {
          setASTNodeTranspiled(
              ast_node, snprintf_concat_float(ast_node->transpiled, "%Lf",
                                              ast_node->left->value.f));
        }
        break;
      default:
        break;
      ...
      case V_INT:
        if (in(operator, logical_operators, logical_operators_size)) {
          setASTNodeTranspiled(
              ast_node, snprintf_concat_string(
                            ast_node->transpiled, "%s",
                            ast_node->right->value.i ? "true" : "false"));
        } else {
          setASTNodeTranspiled(ast_node,
                               snprintf_concat_int(ast_node->transpiled, "%lld",
                                                   ast_node->right->value.i));
      ...
      case V_FLOAT:
        if (in(operator, logical_operators, logical_operators_size)) {
          setASTNodeTranspiled(
              ast_node, snprintf_concat_string(
                            ast_node->transpiled, "%s",
                            ast_node->right->value.f ? "true" : "false"));
        } else {
          setASTNodeTranspiled(
              ast_node, snprintf_concat_float(ast_node->transpiled, "%Lf",
                                              ast_node->right->value.f));
    ...
    if (ast_node->left != NULL && ast_node->left->is_transpiled) {
      ...
      if (ast_node->left->value_type == V_INT) {
        setASTNodeTranspiled(
            ast_node,
            snprintf_concat_int(ast_node->transpiled, "%lld",
                                (long double)ast_node->left->value.i));
      } else {
        setASTNodeTranspiled(ast_node,
                             snprintf_concat_float(ast_node->transpiled, "%Lf",
                                                   ast_node->left->value.f));
      }
    }
    setASTNodeTranspiled(ast_node, strcat_ext(ast_node->transpiled, " "));
    ...
    if (ast_node->right != NULL && ast_node->right->is_transpiled) {
      ...
      if (ast_node->right->value_type == V_INT) {
        setASTNodeTranspiled(
            ast_node,
            snprintf_concat_int(ast_node->transpiled, "%lld",
                                (long double)ast_node->right->value.i));
      } else {
        setASTNodeTranspiled(ast_node,
                             snprintf_concat_float(ast_node->transpiled, "%Lf",
                                                   ast_node->right->value.f));
...
void transpile_function_call(FILE *c_fp, char *module, char *name,
  ...
  free(module_context);
  fprintf(c_fp, "%*ccallFunctionCleanUp(function_call_%llu, %s);\n", indent,
          ' ', compiler_function_counter,
...
void transpile_function_call_create_var(FILE *c_fp, ASTNode *ast_node,
  ...
  fprintf(
      ...
      "%*cupdateDecisionSymbolChainScope();\n"
      "%*cfree(function_call_%llu);\n",
      indent, ' ', ast_node->strings[0], type_strings[type1],
      compiler_function_counter, type_strings[type2], indent, ' ', indent, ' ',
      compiler_function_counter);
}
...
void compiler_handleModuleImportRegister(char *module_name,
  ...
  if (strcmp(get_filename_ext(module_path),
    ...
    spells_dir_path =
        snprintf_concat_string(spells_dir_path, "%s", __KAOS_BUILD_DIRECTORY__);
    spells_dir_path =
        snprintf_concat_string(spells_dir_path, "%s", __KAOS_PATH_SEPARATOR__);
    spells_dir_path =
        snprintf_concat_string(spells_dir_path, "%s", __KAOS_SPELLS__);
    ...
    extension_dir_path = snprintf_concat_string(extension_dir_path, "%s",
                                                __KAOS_PATH_SEPARATOR__);
    extension_dir_path =
        snprintf_concat_string(extension_dir_path, "%s", extension_name);
    ...
    new_module_path =
        snprintf_concat_string(new_module_path, "%s", __KAOS_PATH_SEPARATOR__);
    new_module_path =
        snprintf_concat_string(new_module_path, "%s", extension_name);
    new_module_path = snprintf_concat_string(new_module_path, "%s", ".");
    new_module_path = snprintf_concat_string(
        new_module_path, "%s", (char *)get_filename_ext(module_path));
    ...
    extension_counter++;
    fprintf(c_fp,
            "%*cchar *extension_path_%llu = strcat_ext(getParentDir(argv0), "
            "\"%s%s%s%s%s%s.%s\");\n",
            indent, ' ', extension_counter, __KAOS_PATH_SEPARATOR_COMPILER__,
            __KAOS_SPELLS__, __KAOS_PATH_SEPARATOR_COMPILER__, extension_name,
            __KAOS_PATH_SEPARATOR_COMPILER__, extension_name,
            get_filename_ext(module_path));
    fprintf(c_fp, "%*cpushModuleStack(extension_path_%llu, \"%s\");\n", indent,
...
char *compiler_getFunctionModuleContext(char *name, char *module) {
  if (module == NULL && transpiled_modules.size > 0 &&
      strcmp(transpiled_modules.arr[transpiled_modules.size - 1], "") != 0) {
    char *module_context =
        malloc(strlen(transpiled_modules.arr[transpiled_modules.size - 1]) + 1);
...
bool isFunctionFromDynamicLibrary(char *name, char *module) {
  if (module == NULL && transpiled_modules.size > 0 &&
      strcmp(transpiled_modules.arr[transpiled_modules.size - 1], "") != 0) {
    return strcmp(get_filename_ext(
                      transpiled_modules.arr[transpiled_modules.size - 1]),
                  __KAOS_DYNAMIC_LIBRARY_EXTENSION__) == 0;
  }
  _Function *function = getFunction(name, module);
  return strcmp(get_filename_ext(function->module_context),
                __KAOS_DYNAMIC_LIBRARY_EXTENSION__) == 0;
}
...
void free_transpiled_functions() {
  ...
  if (transpiled_functions.size > 0)
    free(transpiled_functions.arr);
  ...
  if (transpiled_modules.size > 0)
    free(transpiled_modules.arr);
...
void free_transpiled_decisions() {
  ...
  if (transpiled_decisions.size > 0)
    free(transpiled_decisions.arr);
