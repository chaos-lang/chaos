...
char *suggestions[1000] = {"exit",     "quit",
                           "print",    "true",
                           "false",    "symbol_table",
                           "del",      "return",
                           "times do", "end",
...
int esc_key_pressed(int count, int key) { return ctrl_d_key_pressed(); }
...
int shell_readline(char *buf) {
  ...
  if (is_interactive &&
      (loops_inside_function_counter > 0 || is_complex_parsing)) {
