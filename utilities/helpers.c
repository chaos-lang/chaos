...
void prepend_to_array(string_array *array, char *str) {
  array->arr =
      (char **)realloc(array->arr, (array->capacity += 1) * sizeof(char *));
...
void append_to_array(string_array *array, char *str) {
  if (array->capacity == 0) {
    ...
    array->arr =
        (char **)realloc(array->arr, (array->capacity *= 2) * sizeof(char *));
  }
...
char *str_replace(char *target, const char *needle, const char *replacement) {
  ...
  char *new_target = (char *)malloc(
      (1 + strlen(target) + counter * (repl_len - needle_len)) * sizeof(char));
