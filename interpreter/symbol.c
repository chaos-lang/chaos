...
char* type_names[] = {"Boolean", "Number",     "String", "Any",
                      "List",    "Dictionary", "Void"};

char* value_type_names[] = {"Boolean", "Number", "Number", "String", "Void"};
...
Symbol* addSymbol(char* name, enum Type type, union Value value,
  ...
  if (isComplexMode() &&
      complex_mode_stack.arr[complex_mode_stack.size - 1]->type == K_DICT) {
    if (name != NULL) {
...
Symbol* deepCopyComplex(char* name, Symbol* symbol) {
  if (symbol->type == K_LIST) {
    ...
    append_to_array_without_malloc(&free_string_stack, name);
    throw_error(E_UNRECOGNIZED_COMPLEX_DATA_TYPE, getTypeName(symbol->type),
                name);
  }
...
char* _getSymbolValueString(Symbol* symbol) {
  ...
  if (symbol->value_type == V_STRING) {
    ...
    throw_error(E_UNEXPECTED_VALUE_TYPE, symbol->name, NULL, 0,
                symbol->value_type);
  }
  return "";
}
...
void printSymbolValue(Symbol* symbol, bool is_complex, bool pretty,
                      bool escaped, unsigned long iter) {
  char* encoded = NULL;
  encoded = encodeSymbolValueToString(symbol, is_complex, pretty, escaped, iter,
                                      encoded, false);
  printf("%s", encoded);
  free(encoded);
}

char* encodeSymbolValueToString(Symbol* symbol, bool is_complex, bool pretty,
                                bool escaped, unsigned long iter, char* encoded,
                                bool double_quotes) {
  switch (symbol->type) {
  case K_BOOL:
    switch (symbol->value_type) {
  ...
  case K_LIST:
    ...
    for (unsigned long i = 0; i < symbol->children_count; i++) {
      ...
      encoded =
          encodeSymbolValueToString(symbol->children[i], true, pretty, escaped,
                                    iter, encoded, double_quotes);
  ...
  case K_DICT:
    ...
    for (unsigned long i = 0; i < symbol->children_count; i++) {
      ...
      encoded = encodeSymbolValueToString(child, true, pretty, escaped, iter,
                                          encoded, double_quotes);
      if (i + 1 != symbol->children_count) {
        if (pretty) {
          encoded = strcat_ext(encoded, ",\n");
        } else {
  ...
  case K_ANY:
    ...
    return encoded;
  default:
    throw_error(E_UNKNOWN_VARIABLE_TYPE, getTypeName(symbol->type),
                symbol->name);
    break;
  }
  return encoded;
...
bool isDefined(char* name) {
  ...
  symbol_cursor = scope->start_symbol;
  while (symbol_cursor != NULL) {
    if (symbol_cursor->name != NULL && name != NULL &&
...
void addSymbolToComplex(Symbol* symbol) {
  ...
  complex_mode_stack.arr[complex_mode_stack.size - 1]->children = realloc(
      ...
      sizeof(Symbol) *
          ++complex_mode_stack.child_counter[complex_mode_stack.size - 1]);

  if (complex_mode_stack.arr[complex_mode_stack.size - 1]->children == NULL) {
    throw_error(E_MEMORY_ALLOCATION_FOR_LIST_FAILED,
                complex_mode_stack.arr[complex_mode_stack.size - 1]->name);
  }

  complex_mode_stack.arr[complex_mode_stack.size - 1]
      ->children[complex_mode_stack.child_counter[complex_mode_stack.size - 1] -
                 1] = symbol;
}
...
void printSymbolTable() {
  for (unsigned i = 0; i < function_call_stack.size + 1; i++) {
    ...
    while (symbol != NULL) {
      char* encoded = NULL;
      encoded = encodeSymbolValueToString(symbol, false, false, true, 0,
                                          encoded, false);
      FunctionCall* scope = symbol->scope;
      printf(
          "\t\t{id: %llu, name: %s, 2nd_name: %s, key: %s, scope: %s, type: "
          "%u, 2nd_type: %u, value_type: %u, role: %u, param_of: %s, value: "
          "%s} =>\n",
          symbol->id, symbol->name, symbol->secondary_name, symbol->key,
          scope->function->name, symbol->type, symbol->secondary_type,
          symbol->value_type, symbol->role,
          symbol->param_of != NULL ? symbol->param_of->name : "", encoded);
      symbol = symbol->next;
      free(encoded);
    }
    printf("\t[end]\n");
...
Symbol* createCloneFromSymbolByName(char* clone_name, enum Type type,
                                    char* name, enum Type extra_type) {
  Symbol* symbol = getSymbol(name);
  Symbol* clone_symbol =
      createCloneFromSymbol(clone_name, type, symbol, extra_type);
  return clone_symbol;
}

Symbol* createCloneFromComplexElement(char* clone_name, enum Type type,
                                      char* name, enum Type extra_type) {
...
Symbol* createCloneFromSymbol(char* clone_name, enum Type type, Symbol* symbol,
                              enum Type extra_type) {
  if (type != K_ANY && symbol->type != K_ANY && symbol->type != type) {
    throw_error(E_ILLEGAL_VARIABLE_TYPE_FOR_VARIABLE, getTypeName(type),
                clone_name);
  }
  ...
  if (symbol->type == K_LIST || symbol->type == K_DICT) {
    if (symbol->secondary_type != extra_type) {
      throw_error(E_ILLEGAL_VARIABLE_TYPE_FOR_VARIABLE, getTypeName(extra_type),
                  clone_name);
    }
...
Symbol* updateSymbolByClonning(char* clone_name, Symbol* symbol) {
  ...
  if (clone_symbol->type != K_ANY && symbol->type != K_ANY &&
      clone_symbol->type != symbol->type) {
    append_to_array_without_malloc(&free_string_stack, clone_name);
    throw_error(E_ILLEGAL_VARIABLE_TYPE_FOR_VARIABLE, getTypeName(symbol->type),
                clone_name);
  }
...
enum Type isComplexIllegal(enum Type type) {
  if (isComplexMode() && type != K_ANY) {
    for (unsigned long i = 0;
         i <
         complex_mode_stack.arr[complex_mode_stack.size - 1]->children_count;
         i++) {
      Symbol* symbol =
          complex_mode_stack.arr[complex_mode_stack.size - 1]->children[i];
      if (symbol->type != type) {
        return symbol->type;
      }
    }
...
Symbol* finishComplexMode(char* name, enum Type type) {
  ...
  if (illegal_type != (enum Type) - 1) {
    if (name != NULL)
      free(name);
    throw_error(E_ILLEGAL_ELEMENT_TYPE_FOR_TYPED_LIST,
                getTypeName(illegal_type), complex_mode->name);
  }
  popComplexModeStack();
  return complex_mode;
}
...
void _finishComplexModeWithUpdate(Symbol* symbol) {
  ...
  symbol->children =
      realloc(symbol->children, sizeof(Symbol) * complex_mode->children_count);
  memcpy(symbol->children, complex_mode->children,
         complex_mode->children_count * sizeof(Symbol));
...
Symbol* getListElement(Symbol* symbol, long long i) {
  ...
  if (symbol->type != K_LIST)
    throw_error(E_VARIABLE_IS_NOT_AN_LIST, symbol->name);
...
Symbol* getComplexElement(Symbol* complex, long long i, char* key) {
  ...
  if (complex->type == K_LIST || complex->type == K_STRING) {
    ...
    throw_error(E_UNRECOGNIZED_COMPLEX_DATA_TYPE, getTypeName(complex->type),
                complex->name);
  }
...
Symbol* getComplexElementBySymbolId(Symbol* complex,
...
void updateComplexElementComplex() {
  ...
  complex =
      getComplexElementBySymbolId(complex, variable_complex_element_symbol_id);
  _finishComplexModeWithUpdate(complex);
}

void updateComplexElementWrapper(enum Type type, union Value value,
                                 enum ValueType value_type) {
  Symbol* complex = variable_complex_element;
...
void updateComplexElement(Symbol* complex, unsigned long long symbol_id,
  ...
  if (complex->type == K_STRING) {
    ...
    free(value.s);
    return;
  } else if (complex->secondary_type != K_ANY &&
             complex->secondary_type != type) {
    free(key);
    if (type == K_STRING)
      free(value.s);
    throw_error(E_ILLEGAL_ELEMENT_TYPE_FOR_TYPED_LIST, getTypeName(type),
                complex->name);
  }
...
void _updateComplexElementSymbol(Symbol* complex, unsigned long long symbol_id,
                                 Symbol* source) {
  Symbol* access_symbol = getSymbolById(symbol_id);

  if (access_symbol->type == K_LIST || access_symbol->type == K_DICT) {
    throw_error(E_UNEXPECTED_ACCESSOR_DATA_TYPE,
                getTypeName(access_symbol->type), complex->name);
  }
  ...
  if (complex->secondary_type != K_ANY &&
      complex->secondary_type != source->type) {
    removeSymbol(access_symbol);
    free(key);
    throw_error(E_ILLEGAL_ELEMENT_TYPE_FOR_TYPED_LIST,
                getTypeName(source->type), complex->name);
  }
...
void removeComplexElement(Symbol* complex, unsigned long long symbol_id) {
  ...
  if (i != (long long)complex->children_count - 1)
    memcpy(temp + i, complex->children + i + 1,
           (complex->children_count - i - 1) * sizeof(Symbol));
...
Symbol* getSymbolFunctionParameter(char* name) {
  ...
  Symbol* clone_symbol =
      createCloneFromSymbol(NULL, symbol->type, symbol, symbol->secondary_type);
  scope_override = NULL;
  return clone_symbol;
}
...
char* getTypeName(unsigned i) { return type_names[i]; }

char* getValueTypeName(unsigned i) { return value_type_names[i]; }

void pushComplexModeStack(Symbol* complex_mode) {
  if (complex_mode_stack.capacity == 0) {
    complex_mode_stack.arr =
        (Symbol**)malloc((complex_mode_stack.capacity = 2) * sizeof(Symbol));
    complex_mode_stack.child_counter = (unsigned long*)malloc(
        (complex_mode_stack.capacity = 2) * sizeof(unsigned long*));
  } else if (complex_mode_stack.capacity == complex_mode_stack.size) {
    complex_mode_stack.arr =
        (Symbol**)realloc(complex_mode_stack.arr,
                          (complex_mode_stack.capacity *= 2) * sizeof(Symbol));
    complex_mode_stack.child_counter = (unsigned long*)realloc(
        complex_mode_stack.child_counter,
        (complex_mode_stack.capacity *= 2) * sizeof(unsigned long*));
  }
...
bool isNestedComplexMode() { return complex_mode_stack.size > 1; }
...
void pushLeftRightBracketStack(unsigned long long symbol_id) {
  ...
  if (left_right_bracket_stack.capacity == 0) {
    left_right_bracket_stack.arr = (unsigned long long*)malloc(
        (left_right_bracket_stack.capacity += 1) * sizeof(unsigned long long*));
    left_right_bracket_stack.arr[0] = symbol_id;
    left_right_bracket_stack.size++;
    return;
  } else {
    left_right_bracket_stack.arr = (unsigned long long*)realloc(
        left_right_bracket_stack.arr,
        (left_right_bracket_stack.capacity += 1) * sizeof(unsigned long long*));
  }
...
unsigned long long popLeftRightBracketStack() {
  unsigned long long symbol_id =
      left_right_bracket_stack.arr[left_right_bracket_stack.size - 1];
  left_right_bracket_stack.size--;
  return symbol_id;
}
...
Symbol* getComplexElementThroughLeftRightBracketStack(
    char* name, unsigned long inverse_nested) {
  Symbol* symbol = getSymbol(name);
  ...
  for (signed k = (signed short)(left_right_bracket_stack.size - 1);
       k > (-1 + (signed)inverse_nested); k--) {
...
void buildVariableComplexElement(char* name, char* key) {
  variable_complex_element =
      getComplexElementThroughLeftRightBracketStack(name, 1);
  variable_complex_element_symbol_id = popLeftRightBracketStack();

  if (isComplexMode()) {
    Symbol* symbol = variable_complex_element;
    symbol =
        getComplexElementBySymbolId(symbol, variable_complex_element_symbol_id);

    Symbol* clone_symbol = createCloneFromSymbol(NULL, symbol->type, symbol,
...
void pushNestedComplexModeStack(Symbol* complex_mode) {
  ...
  if (nested_complex_mode_stack.capacity == 0) {
    nested_complex_mode_stack.arr = (Symbol**)malloc(
        (nested_complex_mode_stack.capacity = 2) * sizeof(Symbol));
  } else if (nested_complex_mode_stack.capacity ==
             nested_complex_mode_stack.size) {
    nested_complex_mode_stack.arr = (Symbol**)realloc(
        nested_complex_mode_stack.arr,
        (nested_complex_mode_stack.capacity *= 2) * sizeof(Symbol));
  }
...
void popNestedComplexModeStack(char* key) {
  nested_complex_mode_stack.arr[nested_complex_mode_stack.size - 1]->key =
      malloc(1 + strlen(key));
  strcpy(nested_complex_mode_stack.arr[nested_complex_mode_stack.size - 1]->key,
         key);
...
void reverseComplexMode() {
  ...
  complex_mode->children_count =
      complex_mode_stack.child_counter[complex_mode_stack.size - 1];
  if (complex_mode->children_count == 0)
    return;
  unsigned long i = 0;
...
bool resolveRelEqualUnknown(char* name_l, char* name_r) {
  ...
  case V_FLOAT:
    ...
    break;
  default:
    throw_error(E_UNEXPECTED_VALUE_TYPE, symbol_l->name, NULL, 0,
                symbol_l->value_type);
    break;
  }
  return 0;
