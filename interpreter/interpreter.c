...
bool is_node_function_related(ASTNode* ast_node) {
  return (ast_node->node_type >= AST_DEFINE_FUNCTION_BOOL &&
          ast_node->node_type <= AST_DEFINE_FUNCTION_VOID) ||
         (ast_node->node_type >= AST_FUNCTION_PARAMETERS_START &&
          ast_node->node_type <= AST_OPTIONAL_FUNCTION_PARAMETER_STRING_DICT) ||
         (ast_node->node_type >= AST_ADD_FUNCTION_NAME &&
          ast_node->node_type <= AST_MODULE_IMPORT_PARTIAL);
}
...
register_functions_label:
  ...
  if (debug_enabled)
    printf(
        "(Register)\tASTNode: {id: %llu, node_type: %s, module: %s, "
        "string_size: %zu, lineno: %d}\n",
  ...
  case AST_OPTIONAL_FUNCTION_PARAMETER_BOOL:
    addFunctionOptionalParameterBool(ast_node->strings[0],
                                     ast_node->right->value.b);
    break;
  case AST_OPTIONAL_FUNCTION_PARAMETER_NUMBER:
    if (ast_node->right->value_type == V_INT) {
      addFunctionOptionalParameterInt(ast_node->strings[0],
                                      ast_node->right->value.i);
    } else {
      addFunctionOptionalParameterFloat(ast_node->strings[0],
  ...
  if (ast_node->node_type >= AST_DEFINE_FUNCTION_BOOL &&
      ast_node->node_type <= AST_DEFINE_FUNCTION_VOID &&
      function_mode != NULL) {
...
eval_node_label:
  ...
  if (debug_enabled)
    printf(
        "(Execute)\tASTNode: {id: %llu, node_type: %s, module: %s, "
        "string_size: %zu, lineno: %d}\n",
        ast_node->id, getAstNodeTypeName(ast_node->node_type), ast_node->module,
        ast_node->strings_size, ast_node->lineno);
  ...
  case AST_START_TIMES_DO_VAR:
    return startTimesDo(getSymbolValueInt(ast_node->strings[0]), false,
                        ast_node);
    break;
  ...
  case AST_START_FOREACH_DICT:
    return startForeachDict(ast_node->strings[0], ast_node->strings[1],
                            ast_node->strings[2], ast_node);
    break;
  default:
    break;
  }
  ...
  case AST_VAR_CREATE_BOOL_VAR:
    createCloneFromSymbolByName(ast_node->strings[0], K_BOOL,
                                ast_node->strings[1], K_ANY);
    break;
  case AST_VAR_CREATE_BOOL_VAR_EL:
    createCloneFromComplexElement(ast_node->strings[0], K_BOOL,
                                  ast_node->strings[1], K_ANY);
    break;
  ...
  case AST_VAR_CREATE_NUMBER_VAR:
    createCloneFromSymbolByName(ast_node->strings[0], K_NUMBER,
                                ast_node->strings[1], K_ANY);
    break;
  case AST_VAR_CREATE_NUMBER_VAR_EL:
    createCloneFromComplexElement(ast_node->strings[0], K_NUMBER,
                                  ast_node->strings[1], K_ANY);
    break;
  ...
  case AST_VAR_CREATE_STRING_VAR:
    createCloneFromSymbolByName(ast_node->strings[0], K_STRING,
                                ast_node->strings[1], K_ANY);
    break;
  case AST_VAR_CREATE_STRING_VAR_EL:
    createCloneFromComplexElement(ast_node->strings[0], K_STRING,
                                  ast_node->strings[1], K_ANY);
    break;
  ...
  case AST_VAR_CREATE_ANY_VAR:
    createCloneFromSymbolByName(ast_node->strings[0], K_ANY,
                                ast_node->strings[1], K_ANY);
    break;
  case AST_VAR_CREATE_ANY_VAR_EL:
    createCloneFromComplexElement(ast_node->strings[0], K_ANY,
                                  ast_node->strings[1], K_ANY);
    break;
  ...
  case AST_VAR_CREATE_LIST_VAR:
    createCloneFromSymbolByName(ast_node->strings[0], K_LIST,
                                ast_node->strings[1], K_ANY);
    break;
  ...
  case AST_VAR_CREATE_DICT_VAR:
    createCloneFromSymbolByName(ast_node->strings[0], K_DICT,
                                ast_node->strings[1], K_ANY);
    break;
  ...
  case AST_VAR_UPDATE_VAR_EL:
    updateSymbolByClonningComplexElement(ast_node->strings[0],
                                         ast_node->strings[1]);
    break;
  ...
  case AST_PRINT_COMPLEX_EL:
    printSymbolValueEndWithNewLine(
        getComplexElementBySymbolId(variable_complex_element,
                                    variable_complex_element_symbol_id),
        false, false);
    break;
  ...
  case AST_COMPLEX_EL_UPDATE_VAR_EL:
    updateComplexElementSymbol(
        getComplexElementThroughLeftRightBracketStack(ast_node->strings[0], 0));
    break;
  case AST_COMPLEX_EL_UPDATE_FUNC_RETURN:
    ...
    break;
  case AST_PRINT_VAR:
    printSymbolValueEndWithNewLine(getSymbol(ast_node->strings[0]), false,
                                   true);
    break;
...
#ifndef CHAOS_COMPILER
    if (is_interactive)
      if (ast_node->right->node_type < AST_VAR_EXPRESSION_INCREMENT ||
          ast_node->right->node_type > AST_VAR_EXPRESSION_ASSIGN_INCREMENT)
        printf("%lld\n", ast_node->right->value.i);
#endif
    break;
  ...
  case AST_VAR_EXPRESSION_INCREMENT_ASSIGN:
    ast_node->value.i =
        incrementThenAssign(ast_node->strings[0], ast_node->right->value.i);
    ast_node->value_type = V_INT;
    break;
  case AST_VAR_EXPRESSION_ASSIGN_INCREMENT:
    ast_node->value.i =
        assignThenIncrement(ast_node->strings[0], ast_node->right->value.i);
    ast_node->value_type = V_INT;
    break;
  ...
  case AST_BOOLEAN_EXPRESSION_REL_EQUAL_UNKNOWN:
    ast_node->value.b =
        resolveRelEqualUnknown(ast_node->strings[0], ast_node->strings[1]);
    break;
  case AST_BOOLEAN_EXPRESSION_REL_NOT_EQUAL_UNKNOWN:
    ast_node->value.b =
        resolveRelNotEqualUnknown(ast_node->strings[0], ast_node->strings[1]);
    break;
  case AST_BOOLEAN_EXPRESSION_REL_GREAT_UNKNOWN:
    ast_node->value.b =
        resolveRelGreatUnknown(ast_node->strings[0], ast_node->strings[1]);
    break;
  case AST_BOOLEAN_EXPRESSION_REL_SMALL_UNKNOWN:
    ast_node->value.b =
        resolveRelSmallUnknown(ast_node->strings[0], ast_node->strings[1]);
    break;
  case AST_BOOLEAN_EXPRESSION_REL_GREAT_EQUAL_UNKNOWN:
    ast_node->value.b =
        resolveRelGreatEqualUnknown(ast_node->strings[0], ast_node->strings[1]);
    break;
  case AST_BOOLEAN_EXPRESSION_REL_SMALL_EQUAL_UNKNOWN:
    ast_node->value.b =
        resolveRelSmallEqualUnknown(ast_node->strings[0], ast_node->strings[1]);
    break;
  ...
  case AST_LEFT_RIGHT_BRACKET_VAR:
    disable_complex_mode = true;
    symbol =
        createCloneFromSymbolByName(NULL, K_ANY, ast_node->strings[0], K_ANY);
    symbol->sign = 1;
    pushLeftRightBracketStack(symbol->id);
    disable_complex_mode = false;
    break;
  ...
  case AST_DECISION_MAKE_BOOLEAN:
    if (ast_node->right->value.b) {
      ...
      case 1:
        function_call =
            callFunction(ast_node->strings[0],
                         function_call_stack.arr[function_call_stack.size - 1]
                             ->function->module);
        break;
      case 2:
        function_call =
            callFunction(ast_node->strings[1], ast_node->strings[0]);
        break;
      default:
        break;
      }
...
ASTNode* eval_node_after_function_call(ASTNode* end_node) {
  if (function_call_stack.size == 0 ||
      (loop_end_ast_node != NULL && loop_end_ast_node == end_node))
    return NULL;
  ...
  if (debug_enabled)
    printf(
        "(Execute[AFC])\tASTNode: {id: %llu, node_type: %s, module: %s, "
        "string_size: %zu, lineno: %d}\n",
        ast_node->id, getAstNodeTypeName(ast_node->node_type), ast_node->module,
        ast_node->strings_size, ast_node->lineno);
  ...
  switch (ast_node->node_type) {
  case AST_VAR_CREATE_BOOL_FUNC_RETURN:
    createCloneFromFunctionReturn(ast_node->strings[0], K_BOOL, function_call,
                                  K_ANY);
    break;
  case AST_VAR_CREATE_NUMBER_FUNC_RETURN:
    createCloneFromFunctionReturn(ast_node->strings[0], K_NUMBER, function_call,
                                  K_ANY);
    break;
  case AST_VAR_CREATE_STRING_FUNC_RETURN:
    createCloneFromFunctionReturn(ast_node->strings[0], K_STRING, function_call,
                                  K_ANY);
    break;
  case AST_VAR_CREATE_ANY_FUNC_RETURN:
    createCloneFromFunctionReturn(ast_node->strings[0], K_ANY, function_call,
                                  K_ANY);
    break;
  case AST_VAR_CREATE_LIST_FUNC_RETURN:
    createCloneFromFunctionReturn(ast_node->strings[0], K_LIST, function_call,
                                  K_ANY);
    break;
  case AST_VAR_CREATE_DICT_FUNC_RETURN:
    createCloneFromFunctionReturn(ast_node->strings[0], K_DICT, function_call,
                                  K_ANY);
    break;
  case AST_VAR_CREATE_BOOL_LIST_FUNC_RETURN:
    createCloneFromFunctionReturn(ast_node->strings[0], K_LIST, function_call,
...
#ifndef CHAOS_COMPILER
    if (is_interactive &&
        !isFunctionType(ast_node->strings[0], _module, K_VOID) &&
        !inject_mode && !decision_execution_mode) {
      printFunctionReturn(function_call, "\n", false, false);
    } else {
#endif
