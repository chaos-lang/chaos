...
void startFunction(char* name, enum Type type, enum Type secondary_type,
                   char* context, char* module_context, char* module,
                   bool is_dynamic) {
#else
void startFunction(char* name, enum Type type, enum Type secondary_type) {
#endif
...
#ifdef CHAOS_COMPILER
  if (!is_dynamic) {
    function_mode->context = malloc(
        1 + strlen(context_is_module_context ? module_context : context));
    strcpy(function_mode->context,
           context_is_module_context ? module_context : context);
    ...
    if (module_path_stack.size > 1)
      parent_context = 2;
    char* context =
        module_path_stack.arr[module_path_stack.size - parent_context];
  ...
  function_mode->is_dynamic =
      strcmp(get_filename_ext(function_mode->module_context),
             __KAOS_DYNAMIC_LIBRARY_EXTENSION__) == 0;

  if (function_mode->is_dynamic) {
    ASTNode* fake_ast_end;
    fake_ast_end = (struct ASTNode*)calloc(
        1, sizeof(ASTNode) + 0 * sizeof *fake_ast_end->strings);
    fake_ast_end->id = 0;
  ...
  function_mode->parameters =
      realloc(function_mode->parameters,
              sizeof(Symbol) * function_parameters_mode->parameter_count);

  if (function_parameters_mode->parameters != NULL) {
    memcpy(function_mode->parameters, function_parameters_mode->parameters,
  ...
  function_mode->optional_parameter_count =
...
FunctionCall* callFunction(char* name, char* module) {
  ...
  for (unsigned short i = 0; i < function->parameter_count; i++) {
    ...
    if ((i + 1) > function_parameters_mode->parameter_count) {
      ...
      Symbol* parameter_call =
          createCloneFromSymbol(parameter->secondary_name, parameter->type,
                                parameter, parameter->secondary_type);
      parameter_call->scope = function_call;
      parameter_call->param_of = function;

      function_parameters_mode
          ->parameters[function_parameters_mode->parameter_count - 1] =
          parameter_call;
    } else {
      ...
      if (parameter->type != K_ANY && parameter->type != parameter_call->type) {
        resetFunctionParametersMode();
        free(function_call);
        throw_error(E_ILLEGAL_VARIABLE_TYPE_FOR_FUNCTION_PARAMETER,
                    parameter->secondary_name, function->name);
      }
  ...
  function_call_stack.arr[function_call_stack.size - 1]->parent_scope =
      parent_scope;

#ifndef CHAOS_COMPILER
  if (strcmp(module_path_stack.arr[module_path_stack.size - 1],
             function_call_stack.arr[function_call_stack.size - 1]
                 ->function->module_context) == 0 &&
      strcmp(module_stack.arr[module_stack.size - 1], "") == 0) {
    function_call->dont_pop_module_stack = true;
  } else {
#endif
    pushModuleStack(function_call_stack.arr[function_call_stack.size - 1]
...
void callFunctionCleanUp(FunctionCall* function_call, bool has_decision) {
  ...
  if (has_decision) {
#endif
    if (!is_loop_breaked && !is_loop_continued) {
  ...
  if (function_call->function->type != K_VOID &&
      function_call->function->symbol == NULL && !is_loop_breaked &&
      !is_loop_continued) {
    throw_error(E_FUNCTION_DID_NOT_RETURN_ANYTHING,
...
_Function* getFunction(char* name, char* module) {
  if (module == NULL && function_call_stack.size > 0 &&
      strcmp(function_call_stack.arr[function_call_stack.size - 1]
                 ->function->module,
             "") != 0)
    module =
        function_call_stack.arr[function_call_stack.size - 1]->function->module;
  ...
  while (function_cursor != NULL) {
    ...
    bool criteria = function_cursor->name != NULL &&
                    strcmp(function_cursor->name, name) == 0;
    criteria =
        criteria &&
        (strcmp(function_cursor->context,
                module_path_stack.arr[module_path_stack.size - 1]) == 0 ||
         strcmp(function_cursor->module_context,
                module_path_stack.arr[module_path_stack.size - 1]) == 0);
    if (module != NULL) {
      criteria = criteria && strcmp(function_cursor->module, module) == 0;
    }
    if (criteria) {
...
_Function* getFunctionByModuleContext(char* name, char* module_context) {
  ...
  if (phase == PROGRAM) {
    ...
    throw_error(E_UNDEFINED_FUNCTION, name,
                (module_context == NULL || strcmp(module_context, "") == 0)
                    ? "<module>"
                    : module_context);
  }
  return NULL;
...
void removeFunctionIfDefined(char* name) {
  ...
  while (function_cursor != NULL) {
    if (strcmp(function_cursor->name, name) == 0 &&
        strcmp(
            function_cursor->context,
            module_path_stack.arr[module_path_stack.size - parent_context]) ==
            0 &&
        strcmp(function_cursor->module_context,
               module_path_stack.arr[module_path_stack.size - 1]) == 0 &&
        strcmp(function_cursor->module,
               module_stack.arr[module_stack.size - 1]) == 0) {
      _Function* function = function_cursor;
#ifndef CHAOS_COMPILER
...
void printFunctionTable() {
  ...
  while (function != NULL) {
    ...
    printf(
        "\t{name: %s, type: %u, parameter_count: %hu, "
        "optional_parameter_count: %hu, decision_length: %u, context: %s, "
        "module_context: %s, module: %s} =>\n",
        function->name, function->type, function->parameter_count,
        function->optional_parameter_count, function->decision_functions.size,
        context_temp, function->module_context, function->module);
...
void returnSymbol(char* name) {
  ...
  if (symbol->type != K_ANY &&
      function_call_stack.arr[function_call_stack.size - 1]->function->type !=
          K_ANY &&
      symbol->type != function_call_stack.arr[function_call_stack.size - 1]
                          ->function->type) {
    throw_error(
        E_ILLEGAL_VARIABLE_TYPE_FOR_FUNCTION, getTypeName(symbol->type),
        function_call_stack.arr[function_call_stack.size - 1]->function->name);
  }
  if (symbol->secondary_type != K_ANY &&
      ...
      symbol->secondary_type !=
          function_call_stack.arr[function_call_stack.size - 1]
              ->function->secondary_type) {
    throw_error(
        E_ILLEGAL_VARIABLE_TYPE_FOR_FUNCTION,
        getTypeName(symbol->secondary_type),
        function_call_stack.arr[function_call_stack.size - 1]->function->name);
  }

  scope_override =
      function_call_stack.arr[function_call_stack.size - 1]->parent_scope;
  function_call_stack.arr[function_call_stack.size - 1]->function->symbol =
...
void printFunctionReturn(FunctionCall* function_call, char* end, bool pretty,
                         bool escaped) {
  ...
  printSymbolValueEndWith(function_call->function->symbol, end, pretty,
                          escaped);
  freeFunctionReturn(function_call);
}

void createCloneFromFunctionReturn(char* clone_name, enum Type type,
                                   FunctionCall* function_call,
  ...
  createCloneFromSymbol(clone_name, type, function_call->function->symbol,
                        extra_type);
  freeFunctionReturn(function_call);
}

void updateSymbolByClonningFunctionReturn(char* clone_name,
                                          FunctionCall* function_call) {
  if (function_call->function->symbol == NULL) {
    throw_error(E_FUNCTION_DID_NOT_RETURN_ANYTHING,
                function_call->function->name);
...
void addBooleanDecision() {
  append_to_array(&decision_mode->decision_functions,
...
void addDefaultDecision() {
  decision_mode->decision_default =
      realloc(decision_mode->decision_default,
              1 + strlen(trim_string(decision_buffer)));
...
void executeDecision(FunctionCall* function_call) {
  ...
  eval_node(function_call->function->decision_node,
            function_call->function->module_context);
  ...
  if (function_call_stack.arr[function_call_stack.size - 1]->function->type !=
          K_VOID &&
      function_call_stack.arr[function_call_stack.size - 1]->function->symbol ==
          NULL) {
    scope_override =
        function_call_stack.arr[function_call_stack.size - 1]->parent_scope;
    function_call_stack.arr[function_call_stack.size - 1]->function->symbol =
        createCloneFromSymbol(NULL, decision_symbol_chain->type,
                              decision_symbol_chain,
...
bool isInFunctionNamesBuffer(char* name) {
  for (unsigned i = 0; i < function_names_buffer.size; i++) {
    if (strcmp(function_names_buffer.arr[i], name) == 0)
      return true;
  }
  return false;
}
...
void pushExecutedFunctionStack(FunctionCall* function_call) {
  if (function_call_stack.capacity == 0) {
    function_call_stack.arr = (FunctionCall**)malloc(
        (function_call_stack.capacity = 2) * sizeof(FunctionCall));
  } else if (function_call_stack.capacity == function_call_stack.size) {
    function_call_stack.arr = (FunctionCall**)realloc(
        function_call_stack.arr,
        (function_call_stack.capacity *= 2) * sizeof(FunctionCall));
  }
...
void updateDecisionSymbolChainScope() {
  if (function_call_stack.size > 0 && decision_symbol_chain != NULL) {
    changeSymbolScope(decision_symbol_chain,
                      function_call_stack.arr[function_call_stack.size - 1]);
  }
}
