...
void moduleImportParse(char *module_path) {
  if (strcmp(get_filename_ext(module_path),
...
char *resolveModulePath(char *module_name, bool directly_import) {
  ...
  module_path =
      malloc(strlen(module_path_stack.arr[module_path_stack.size - 1]) + 1);
  ...
  char *module =
      malloc(strlen(modules_buffer.arr[modules_buffer.size - 1]) + 1);
...
void freeModulesBuffer() {
  ...
  if (modules_buffer.size > 0)
    free(modules_buffer.arr);
  modules_buffer.capacity = 0;
  modules_buffer.size = 0;
}
...
char *searchSpellsIfNotExits(char *module_path, char *relative_path) {
  ...
  if (is_file_exists(module_path)) {
    ...
    if (strlen(spells_dir) > 0)
      spells_dir = strcat_ext(spells_dir, __KAOS_PATH_SEPARATOR__);
    char *spell_name =
        remove_ext(relative_path, '.', __KAOS_PATH_SEPARATOR_ASCII__);
    ...
    if (is_file_exists(module_path)) {
      ...
      char *dynamic_library_path =
          remove_ext(module_path, '.', __KAOS_PATH_SEPARATOR_ASCII__);
      free(module_path);
      dynamic_library_path = strcat_ext(dynamic_library_path, ".");
      dynamic_library_path =
          strcat_ext(dynamic_library_path, __KAOS_DYNAMIC_LIBRARY_EXTENSION__);
