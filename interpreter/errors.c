...
void throw_error_base(unsigned short code, char *str1, char *str2,
                      long long lld1, unsigned long long llu1,
                      bool is_preemptive, _Function *function) {
  ...
  if (is_preemptive) {
    ...
    sprintf(bg_color, "41");
    sprintf(title_msg, "%*c%s Error (most recent call last):", indent, ' ',
            __KAOS_LANGUAGE_NAME__);
  }

  switch (code) {
  case E_UNKNOWN_VARIABLE_TYPE:
    sprintf(error_msg, "Unknown variable type: %s for variable: %s", str1,
            str2);
    break;
  ...
  case E_ILLEGAL_ELEMENT_TYPE_FOR_TYPED_LIST:
    sprintf(error_msg, "Illegal element type: %s for the typed list: %s", str1,
            str2);
    break;
  ...
  case E_ILLEGAL_VARIABLE_TYPE_FOR_VARIABLE:
    sprintf(error_msg, "Illegal variable type: %s, for variable: %s", str1,
            str2);
    break;
  ...
  case E_UNRECOGNIZED_COMPLEX_DATA_TYPE:
    sprintf(error_msg, "Unrecognized complex data type: %s for variable: %s",
            str1, str2);
    break;
  case E_ILLEGAL_VARIABLE_TYPE_FOR_FUNCTION:
    sprintf(error_msg, "Illegal variable type: %s for function: %s", str1,
            str2);
    break;
  ...
  case E_UNEXPECTED_VALUE_TYPE:
    sprintf(error_msg, "Unexpected value type: %llu for variable: %s", llu1,
            str1);
    break;
  ...
  case E_MODULE_IS_EMPTY_OR_NOT_EXISTS_ON_PATH:
    sprintf(error_msg,
            "The module %s is either empty or not exists on the path!", str1);
    break;
  ...
  case E_NOT_A_CHARACTER:
    sprintf(
        error_msg,
        "Right hand side of the assignment is not a character for string: %s",
        str1);
    break;
  case E_PROGRAM_FILE_DOES_NOT_EXISTS_ON_PATH:
    sprintf(error_msg, "Program file does not exists on the given path: %s",
            str1);
    break;
  ...
  case E_ILLEGAL_VARIABLE_TYPE_FOR_FUNCTION_PARAMETER:
    sprintf(error_msg,
            "Illegal variable type for function parameter: %s of function: %s",
            str1, str2);
    break;
  case E_UNEXPECTED_ACCESSOR_DATA_TYPE:
    sprintf(error_msg, "Unexpected accessor data type: %s for variable: %s",
            str1, str2);
    break;
  ...
  case E_BREAK_CALL_OUTSIDE_LOOP:
    sprintf(error_msg,
            "Call to a function with `break` from outside a loop: %s", str1);
    break;
  case E_CONTINUE_CALL_OUTSIDE_LOOP:
    sprintf(error_msg,
            "Call to a function with `continue` from outside a loop: %s", str1);
    break;
  case E_BREAK_CALL_MULTILINE_LOOP:
    sprintf(error_msg,
            "Call to a function with `break` from a multiline loop: %s", str1);
    break;
  case E_CONTINUE_CALL_MULTILINE_LOOP:
    sprintf(error_msg,
            "Call to a function with `continue` from a multiline loop: %s",
            str1);
    break;
  default:
  ...
  if (!is_preemptive) {
    for (unsigned i = 0; i < function_call_stack.size; i++) {
      sprintf(
          traceback_line_msg[i], "%*cFile: \"%s\", line %d, in %s", indent * 2,
          ' ',
          i != 0 ? function_call_stack.arr[i - 1]->function->module_context
      ...
      if (is_interactive &&
          strcmp(module_path_stack.arr[0],
                 i != 0
      ...
      sprintf(traceback_line[i], "%*c%s", indent * 3, ' ', trim(line));
      free(line);
    }
  }
  ...
  sprintf(traceback_line_msg[traceback_size - 1],
          "%*cFile: \"%s\", line %d, in %s", indent * 2, ' ', module_path,
          kaos_lineno, function_name != NULL ? function_name : "<module>");
  ...
  sprintf(traceback_line[traceback_size - 1], "%*c%s", indent * 3, ' ',
          trim(line));
  free(line);
