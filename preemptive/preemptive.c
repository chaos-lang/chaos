...
void preemptive_check() {
  ...
  while (function_cursor != NULL) {
    ...
    if (debug_enabled)
      printf("(CheckF)\tFunction: {name: %s, module: %s, lineno: %d}\n",
             function->name, function->module, function->line_no);
    ...
    if (function->type != K_VOID && function->symbol == NULL &&
        function->decision_node == NULL && !function->is_dynamic &&
        !preemptive_continue) {
      add_preemptive_error(E_FUNCTION_DID_NOT_RETURN_ANYTHING, function,
                           function->name);
    }
...
check_function_label:
  ...
  if (debug_enabled)
    printf(
        "(Check)\t\tASTNode: {id: %llu, node_type: %s, module: %s, "
  ...
  case AST_VAR_CREATE_BOOL_FUNC_RETURN:
    ...
    case 2:
      ...
      break;
    case 3:
      preemptive_callFunction(function, ast_node->strings[2],
                              ast_node->strings[1]);
      break;
    default:
      break;
  ...
  case AST_VAR_CREATE_NUMBER:
    preemptive_addSymbol(ast_node->strings[0], K_NUMBER,
                         ast_node->right->value_type);
    break;
  ...
  case AST_VAR_CREATE_ANY_NUMBER:
    preemptive_addSymbol(ast_node->strings[0], K_ANY,
                         ast_node->right->value_type);
    break;
  ...
  case AST_COMPLEX_EL_UPDATE_FUNC_RETURN:
    ...
    case 1:
      ...
      break;
    case 2:
      preemptive_callFunction(function, ast_node->strings[1],
                              ast_node->strings[0]);
      break;
    default:
      break;
...
check_break_continue_label:
  ...
  if (debug_enabled)
    printf(
        "(CheckBC)\tASTNode: {id: %llu, node_type: %s, module: %s, "
        "string_size: %zu, lineno: %d}\n",
        ast_node->id, getAstNodeTypeName(ast_node->node_type), ast_node->module,
        ast_node->strings_size, ast_node->lineno);
  ...
  case AST_DECISION_MAKE_BOOLEAN_BREAK:
    ...
    if (preemptive_loop_length > 2) {
      add_preemptive_error(E_BREAK_CALL_MULTILINE_LOOP, function,
                           function->name);
      return;
    }
    break;
  case AST_DECISION_MAKE_BOOLEAN_CONTINUE:
    if (nested_loop_counter == 0) {
      add_preemptive_error(E_CONTINUE_CALL_OUTSIDE_LOOP, function,
                           function->name);
      return;
    }
    if (preemptive_loop_length > 2) {
      add_preemptive_error(E_CONTINUE_CALL_MULTILINE_LOOP, function,
                           function->name);
      return;
    }
    break;
...
preemptive_walk_until_end_label:
  ...
  if (strcmp(ast_node->module, module) != 0)
    return preemptive_walk_until_end(ast_node->next, module);
