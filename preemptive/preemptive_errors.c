...
void add_preemptive_error_var(throw_preemptive_error_args in) {
    if (preemptive_errors.capacity == 0) {
        preemptive_errors_lineno.value = (int*)malloc((preemptive_errors_lineno.capacity = 2) * sizeof(int));
        preemptive_errors.arr = (throw_preemptive_error_args*)malloc((preemptive_errors.capacity = 2) * sizeof(throw_preemptive_error_args));
    } else if (preemptive_errors.capacity == preemptive_errors.size) {
        preemptive_errors_lineno.value = (int*)realloc(preemptive_errors_lineno.value, (preemptive_errors_lineno.capacity *= 2) * sizeof(int));
        preemptive_errors.arr = (throw_preemptive_error_args*)realloc(preemptive_errors.arr, (preemptive_errors.capacity *= 2) * sizeof(throw_preemptive_error_args));
    }
