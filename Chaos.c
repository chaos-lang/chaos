...
int defineFunction(char *name, enum Type type, enum Type secondary_type,
                   char *params_name[], unsigned params_type[],
                   unsigned params_secondary_type[],
                   unsigned short params_length, KaosValue optional_params[],
                   unsigned short optional_params_length) {
  startFunctionParameters();

  unsigned short optional_params_starts_from =
      params_length - optional_params_length;

  unsigned short j = optional_params_length - 1;
  if (params_length > 0) {
    for (int i = (int)(params_length - 1); i >= optional_params_starts_from;
         i--) {
      ...
      case K_BOOL:
        addFunctionOptionalParameterBool(params_name[i],
                                         optional_param_value.b);
        break;
      case K_NUMBER:
        addFunctionOptionalParameterFloat(params_name[i],
                                          optional_param_value.f);
        break;
      case K_STRING:
        addFunctionOptionalParameterString(params_name[i],
    ...
    for (int i = (int)optional_params_starts_from - 1; i >= 0; i--) {
      addFunctionParameter(params_name[i], params_type[i],
                           params_secondary_type[i]);
    }
  }
...
bool getVariableBool(char *name) { return getSymbolValueBool(name); }

long long getVariableInt(char *name) { return getSymbolValueInt(name); }

long double getVariableFloat(char *name) { return getSymbolValueFloat(name); }

char *getVariableString(char *name) { return getSymbolValueString(name); }
...
void copyDictElement(char *name, char *key) {
  Symbol *symbol = getDictElement(getSymbol(name), key);
  createCloneFromSymbol(key, symbol->type, symbol, symbol->secondary_type);
}
...
char *dumpVariableToString(char *name, bool pretty, bool escaped,
  ...
  if (symbol->type == K_LIST || symbol->type == K_DICT)
    is_complex = true;
  char *encoded = NULL;
  encoded = encodeSymbolValueToString(symbol, is_complex, pretty, escaped, 0,
...
void createVariableBool(char *name, bool b) { addSymbolBool(name, b); }

void createVariableInt(char *name, long long i) { addSymbolInt(name, i); }

void createVariableFloat(char *name, long double f) { addSymbolFloat(name, f); }

void createVariableString(char *name, char *s) { addSymbolString(name, s); }

void startBuildingList() { addSymbolList(NULL); }

void returnList(enum Type type) { returnComplex(type); }

void startBuildingDict() { addSymbolDict(NULL); }

void returnDict(enum Type type) { returnComplex(type); }
...
void finishList(enum Type type) { finishComplex(type); }

void finishDict(enum Type type) { finishComplex(type); }
